...
class Computer implements IComputer, IOverclock {
  ...
  static generate(): Computer {
    ...
    return new Computer(
      ...
      ["Chrome", "VSCode", "Docker"].slice(0, Math.floor(Math.random() * 3) + 1),
...
const byProcessorAndManufacturer = computers1.filter(
  c => c.processor === ProcessorType.AMD && c.manufacturer === ManufacturerType.HP
);
...
// Extend Computer with events
class ComputerWithEvents extends Computer {
  onUserAdded = new Event<string>();
  onProcessorChanged = new Event<ProcessorType>();
  onSoftwareInstalled = new Event<string>();
  onRamChanged = new Event<number>();

  addUser(user: string) {
    this.users.push(user);
    this.onUserAdded.trigger(user);
  }

  changeProcessor(p: ProcessorType) {
    this.processor = p;
    this.onProcessorChanged.trigger(p);
  }

  installSoftware(soft: string) {
    this.software.push(soft);
    this.onSoftwareInstalled.trigger(soft);
  }

  changeRam(ram: number) {
    this.ram = ram;
    this.onRamChanged.trigger(ram);
  }
}

// ===== USAGE EXAMPLE =====
const pc = new ComputerWithEvents();

const userHandler = (u: string) => console.log("User added:", u);
pc.onUserAdded.add(userHandler);
pc.addUser("newUser");
pc.onUserAdded.remove(userHandler);

// ===== NOTE ABOUT NUnit =====
// TypeScript does NOT support NUnit or dotnet test.
// Equivalent tools:
// - Vitest (recommended)
// - Jest
//
